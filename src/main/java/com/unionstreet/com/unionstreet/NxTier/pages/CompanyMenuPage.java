...
public class CompanyMenuPage {
    ...
    public String RanNumber = RandomStringUtils.randomNumeric(8);
    ...
    public void loadAndAssertServiceChargesCreatedByAPI(String serviceCharge, String CLI, String salePrice, String costPrice, String baseLineCost) {
        utils.waitForElementVisible(By.xpath("//a[contains(text(),'" + serviceCharge + "')]"));
        utils.waitForElementVisible(By.xpath("//td[contains(text(),'" + CLI + "')]"));
        utils.clickBtn(By.xpath("//a[contains(text(),'" + serviceCharge + "')]"));
        utils.switchToNewWindow();
        utils.waitForElementVisible(By.xpath("//input[@id='r_default'][@placeholder='" + salePrice + "']"));
        utils.waitForElementVisible(By.xpath("//input[@id='r_cost_price'][@placeholder='" + costPrice + "']"));
        utils.waitForElementVisible(By.xpath("//input[@id='r_baseline'][@placeholder='" + baseLineCost + "']"));
    ...
    public void assertChargesForResellerAndAgent(String site, String charge, boolean ifReseller, boolean IfAgent) throws InterruptedException {
        ...
        utils.switchToNewWindow();
        if (ifReseller) {
            utils.waitForElementVisible(By.xpath("//label[contains(text(),'Cost Price')]/../../div[2]/input[@value='100.0000']"));
        } else {
        }
        if (IfAgent) {
            utils.waitForElementVisible(By.xpath("//label[contains(text(),'Sales Price')]/../../div[2]/input[@value='100.0000']"));
    ...
    public void assertBillRunWhenLoggedInAsWLR_underCLI(String customerName) throws InterruptedException {
        ...
        utils.waitForElementVisible(By.xpath("//span[text()[contains(.,'customerOfWLR > " + customerName + "')]]"));
        try {
            utils.assertElementNotPresent(By.id(BILLRUN_UNDER_CLI));
        } catch (Exception e) {
