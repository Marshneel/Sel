$(document).ready(function() {var formatter = new CucumberHTML.DOMFormatter($('.cucumber-report'));formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions.feature");
formatter.feature({
  "line": 2,
  "name": "WLR3_permissions@smokePack",
  "description": "",
  "id": "wlr3-permissions@smokepack",
  "keyword": "Feature"
});
formatter.before({
  "duration": 6800472312,
  "status": "passed"
});
formatter.scenario({
  "line": 5,
  "name": "Creating an agent permission group and assigning it to an agent",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-an-agent-permission-group-and-assigning-it-to-an-agent",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 4,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 6,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 7,
  "name": "have added a new business customer with required contact type",
  "keyword": "And "
});
formatter.step({
  "line": 8,
  "name": "have created a new agent permission group",
  "keyword": "And "
});
formatter.step({
  "line": 9,
  "name": "I wish to create agent contact with valid login credentials",
  "keyword": "When "
});
formatter.step({
  "line": 10,
  "name": "the agent permission group should be available for selection under drop down under agentUser creation",
  "keyword": "Then "
});
formatter.step({
  "line": 11,
  "name": "I should be able to login with agent credentials",
  "keyword": "And "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 5155195949,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.haveAddedANewBusinessCustomerWithRequiredContactType()"
});
formatter.result({
  "duration": 7415817545,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.haveCreatedANewAgentPermissionGroup()"
});
formatter.result({
  "duration": 18833514860,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iWishToCreateAgentContactWithValidLoginCredentials()"
});
formatter.result({
  "duration": 15949598477,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.theAgentPermissionGroupShouldBeAvailableForSelectionUnderDropDownUnderAgentUserCreation()"
});
formatter.result({
  "duration": 4087291760,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iShouldBeAbleToLoginWithAgentCredentials()"
});
formatter.result({
  "duration": 7155761360,
  "status": "passed"
});
formatter.after({
  "duration": 647491214,
  "status": "passed"
});
formatter.before({
  "duration": 5653925361,
  "status": "passed"
});
formatter.scenario({
  "line": 14,
  "name": "creating a reseller with agent permission group",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-a-reseller-with-agent-permission-group",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 13,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 15,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 16,
  "name": "have added a new business customer with required contact type",
  "keyword": "And "
});
formatter.step({
  "line": 17,
  "name": "have created a new agent permission group",
  "keyword": "And "
});
formatter.step({
  "line": 18,
  "name": "I wish to create a business reseller contact with valid login credentials",
  "keyword": "When "
});
formatter.step({
  "line": 19,
  "name": "the agent permission group should be available for selection under drop down under business reseller contact creation",
  "keyword": "Then "
});
formatter.step({
  "line": 20,
  "name": "I should be able to login with reseller credentials",
  "keyword": "And "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 4206109753,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.haveAddedANewBusinessCustomerWithRequiredContactType()"
});
formatter.result({
  "duration": 7191875780,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.haveCreatedANewAgentPermissionGroup()"
});
formatter.result({
  "duration": 19620931717,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iWishToCreateABusinessResellerContactWithValidLoginCredentials()"
});
formatter.result({
  "duration": 11182297311,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.theAgentPermissionGroupShouldBeAvailableForSelectionUnderDropDownUnderBusinessResellerContactCreation()"
});
formatter.result({
  "duration": 4317357345,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iShouldBeAbleToLoginWithResellerCredentials()"
});
formatter.result({
  "duration": 13938451168,
  "status": "passed"
});
formatter.after({
  "duration": 506503578,
  "status": "passed"
});
formatter.before({
  "duration": 5638644186,
  "status": "passed"
});
formatter.scenario({
  "line": 23,
  "name": "Creating a CP permission group and assigning it a CP",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-a-cp-permission-group-and-assigning-it-a-cp",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 22,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 24,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 25,
  "name": "Have created a new CP permission group under settings",
  "keyword": "And "
});
formatter.step({
  "line": 26,
  "name": "I wish to create new CP under login users sub-menu",
  "keyword": "When "
});
formatter.step({
  "line": 27,
  "name": "the CP permission should be available for selection under drop down during the CP creation",
  "keyword": "Then "
});
formatter.step({
  "line": 28,
  "name": "I should be able to login with CP credentials",
  "keyword": "And "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 5552122206,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.haveCreatedANewCPPermissionGroupUnderSettings()"
});
formatter.result({
  "duration": 20044569615,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iWishToCreateNewCPUnderLoginUsersSubMenu()"
});
formatter.result({
  "duration": 13799937090,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.theCPPermissionShouldBeAvailableForSelectionUnderDropDownDuringTheCPCreation()"
});
formatter.result({
  "duration": 4494958399,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iShouldBeAbleToLoginWithCPCredentials()"
});
formatter.result({
  "duration": 9392652321,
  "status": "passed"
});
formatter.after({
  "duration": 553444717,
  "status": "passed"
});
formatter.before({
  "duration": 7001244627,
  "status": "passed"
});
formatter.scenario({
  "line": 31,
  "name": "creating an order quote by logged in as an agent and verifying all required fields are visible on the quote screen",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-an-order-quote-by-logged-in-as-an-agent-and-verifying-all-required-fields-are-visible-on-the-quote-screen",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 30,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 32,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 33,
  "name": "Have created a new customer",
  "keyword": "And "
});
formatter.step({
  "line": 34,
  "name": "I access create Quote tab",
  "keyword": "When "
});
formatter.step({
  "line": 35,
  "name": "All required fields pertaining to an agent should be visible on the create quote window",
  "keyword": "Then "
});
formatter.step({
  "line": 36,
  "name": "Only the quotes created by me and the ones created for my company should be visible",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 5184926334,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.haveCreatedANewCustomer()"
});
formatter.result({
  "duration": 7094022649,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAccessCreateQuoteTab()"
});
formatter.result({
  "duration": 7998458329,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.allRequiredFieldsPertainingToAnAgentShouldBeVisibleOnTheCreateQuoteWindow()"
});
formatter.result({
  "duration": 6741297721,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.onlyTheQuotesCreatedByMeAndTheOnesCreatedForMyCompanyShouldBeVisible()"
});
formatter.result({
  "duration": 2937081565,
  "status": "passed"
});
formatter.after({
  "duration": 445047421,
  "status": "passed"
});
formatter.before({
  "duration": 6295644438,
  "status": "passed"
});
formatter.scenario({
  "line": 39,
  "name": "creating an order quote by logged in as a reseller and verifying all required fields are visible on the quote screen",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-an-order-quote-by-logged-in-as-a-reseller-and-verifying-all-required-fields-are-visible-on-the-quote-screen",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 38,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 40,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 41,
  "name": "Have created a new customer",
  "keyword": "And "
});
formatter.step({
  "line": 42,
  "name": "I access create Quote tab",
  "keyword": "When "
});
formatter.step({
  "line": 43,
  "name": "All required fields pertaining to a reseller should be visible on the create quote window",
  "keyword": "Then "
});
formatter.step({
  "line": 44,
  "name": "Only the quotes created by me and the ones created for companies under my contact list should be visible",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 9847662196,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.haveCreatedANewCustomer()"
});
formatter.result({
  "duration": 7460172527,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAccessCreateQuoteTab()"
});
formatter.result({
  "duration": 8139688369,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.allRequiredFieldsPertainingToAResellerShouldBeVisibleOnTheCreateQuoteWindow()"
});
formatter.result({
  "duration": 5988432784,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.onlyTheQuotesCreatedByMeAndTheOnesCreatedForCompaniesUnderMyContactListShouldBeVisible()"
});
formatter.result({
  "duration": 2762203114,
  "status": "passed"
});
formatter.after({
  "duration": 471577939,
  "status": "passed"
});
formatter.before({
  "duration": 5656615664,
  "status": "passed"
});
formatter.scenario({
  "line": 47,
  "name": "creating an order quote by logged in as a CP and verifying all required fields are visible on the quote screen",
  "description": "",
  "id": "wlr3-permissions@smokepack;creating-an-order-quote-by-logged-in-as-a-cp-and-verifying-all-required-fields-are-visible-on-the-quote-screen",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 46,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 48,
  "name": "I am logged in as CP",
  "keyword": "Given "
});
formatter.step({
  "line": 49,
  "name": "I have added a new business customer",
  "keyword": "And "
});
formatter.step({
  "line": 50,
  "name": "I access create Quote tab",
  "keyword": "When "
});
formatter.step({
  "line": 51,
  "name": "All required fields pertaining to a CP should be visible on the create quote window",
  "keyword": "Then "
});
formatter.step({
  "line": 52,
  "name": "all the quotes including that of the resellers and agents should be visible",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsCP()"
});
formatter.result({
  "duration": 10080752560,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iHaveAddedANewBusinessCustomer()"
});
formatter.result({
  "duration": 7602924919,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAccessCreateQuoteTab()"
});
formatter.result({
  "duration": 8157799615,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.allRequiredFieldsPertainingToACPShouldBeVisibleOnTheCreateQuoteWindow()"
});
formatter.result({
  "duration": 6235593204,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.allTheQuotesIncludingThatOfTheResellersAndAgentsShouldBeVisible()"
});
formatter.result({
  "duration": 14490625296,
  "status": "passed"
});
formatter.after({
  "duration": 503515929,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA44_Agent_Login.feature");
formatter.feature({
  "line": 1,
  "name": "Agent login",
  "description": "",
  "id": "agent-login",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5375247461,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "Login as an agent, create a business customer and verify the created customer from the company and site drop downs on create quote page",
  "description": "",
  "id": "agent-login;login-as-an-agent,-create-a-business-customer-and-verify-the-created-customer-from-the-company-and-site-drop-downs-on-create-quote-page",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "Have created a new customer",
  "keyword": "And "
});
formatter.step({
  "line": 7,
  "name": "I access create Quote window",
  "keyword": "When "
});
formatter.step({
  "line": 8,
  "name": "the newly created customer details should appear under the company and site drop downs",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 4402124832,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.haveCreatedANewCustomer()"
});
formatter.result({
  "duration": 6976742403,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.iAccessCreateQuoteWindow()"
});
formatter.result({
  "duration": 6203552577,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.theNewlyCreatedCustomerDetailsShouldAppearUnderTheCompanyAndSiteDropDowns()"
});
formatter.result({
  "duration": 879953680,
  "status": "passed"
});
formatter.after({
  "duration": 419325927,
  "status": "passed"
});
formatter.scenarioOutline({
  "line": 11,
  "name": "Login as a CP, create a business customer, assign an agent and create a quote for the customer. when logged in as the agent the created quote should appear on the orders manager",
  "description": "",
  "id": "agent-login;login-as-a-cp,-create-a-business-customer,-assign-an-agent-and-create-a-quote-for-the-customer.-when-logged-in-as-the-agent-the-created-quote-should-appear-on-the-orders-manager",
  "type": "scenario_outline",
  "keyword": "Scenario Outline",
  "tags": [
    {
      "line": 10,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 12,
  "name": "I am logged in as an agent after the CP has created a quote for a \"\u003ccustomer_name\u003e\" assigned to me",
  "keyword": "Given "
});
formatter.step({
  "line": 13,
  "name": "a quote been created for my \"\u003ccustomer_name\u003e\" by a CP",
  "keyword": "And "
});
formatter.step({
  "line": 14,
  "name": "I access orders manager",
  "keyword": "When "
});
formatter.step({
  "line": 15,
  "name": "I should not see that particular quote",
  "keyword": "Then "
});
formatter.examples({
  "line": 16,
  "name": "",
  "description": "",
  "id": "agent-login;login-as-a-cp,-create-a-business-customer,-assign-an-agent-and-create-a-quote-for-the-customer.-when-logged-in-as-the-agent-the-created-quote-should-appear-on-the-orders-manager;",
  "rows": [
    {
      "cells": [
        "customer_name"
      ],
      "line": 17,
      "id": "agent-login;login-as-a-cp,-create-a-business-customer,-assign-an-agent-and-create-a-quote-for-the-customer.-when-logged-in-as-the-agent-the-created-quote-should-appear-on-the-orders-manager;;1"
    },
    {
      "cells": [
        "businessCustomer"
      ],
      "line": 18,
      "id": "agent-login;login-as-a-cp,-create-a-business-customer,-assign-an-agent-and-create-a-quote-for-the-customer.-when-logged-in-as-the-agent-the-created-quote-should-appear-on-the-orders-manager;;2"
    }
  ],
  "keyword": "Examples"
});
formatter.before({
  "duration": 5581952209,
  "status": "passed"
});
formatter.scenario({
  "line": 18,
  "name": "Login as a CP, create a business customer, assign an agent and create a quote for the customer. when logged in as the agent the created quote should appear on the orders manager",
  "description": "",
  "id": "agent-login;login-as-a-cp,-create-a-business-customer,-assign-an-agent-and-create-a-quote-for-the-customer.-when-logged-in-as-the-agent-the-created-quote-should-appear-on-the-orders-manager;;2",
  "type": "scenario",
  "keyword": "Scenario Outline",
  "tags": [
    {
      "line": 10,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 12,
  "name": "I am logged in as an agent after the CP has created a quote for a \"businessCustomer\" assigned to me",
  "matchedColumns": [
    0
  ],
  "keyword": "Given "
});
formatter.step({
  "line": 13,
  "name": "a quote been created for my \"businessCustomer\" by a CP",
  "matchedColumns": [
    0
  ],
  "keyword": "And "
});
formatter.step({
  "line": 14,
  "name": "I access orders manager",
  "keyword": "When "
});
formatter.step({
  "line": 15,
  "name": "I should not see that particular quote",
  "keyword": "Then "
});
formatter.match({
  "arguments": [
    {
      "val": "businessCustomer",
      "offset": 67
    }
  ],
  "location": "NA44_Agent_Login_stepDefs.iAmLoggedInAsAnAgentAfterTheCPHasCreatedAQuoteForAAssignedToMe(String)"
});
formatter.result({
  "duration": 15007097968,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "businessCustomer",
      "offset": 29
    }
  ],
  "location": "NA44_Agent_Login_stepDefs.aQuoteBeenCreatedForMyByACP(String)"
});
formatter.result({
  "duration": 16168087186,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.iAccessOrdersManager()"
});
formatter.result({
  "duration": 11051448725,
  "status": "passed"
});
formatter.match({
  "location": "NA44_Agent_Login_stepDefs.iShouldNotSeeThatParticularQuote()"
});
formatter.result({
  "duration": 1341033642,
  "status": "passed"
});
formatter.after({
  "duration": 469727148,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA45_Reseller_Login.feature");
formatter.feature({
  "line": 1,
  "name": "Reseller Login",
  "description": "",
  "id": "reseller-login",
  "keyword": "Feature"
});
formatter.before({
  "duration": 6501322812,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "Login as a reseller, create a site and verify the created site from the site drop down on create quote page",
  "description": "",
  "id": "reseller-login;login-as-a-reseller,-create-a-site-and-verify-the-created-site-from-the-site-drop-down-on-create-quote-page",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "have created a new site for a company",
  "keyword": "And "
});
formatter.step({
  "line": 7,
  "name": "I access create Quote tab and select the above company from the company dropdown",
  "keyword": "When "
});
formatter.step({
  "line": 8,
  "name": "The newly created site should appear under the site drop down",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 4539336324,
  "status": "passed"
});
formatter.match({
  "location": "NA45_Reseller_Login_stepDefs.haveCreatedANewSiteForACompany()"
});
formatter.result({
  "duration": 13540664268,
  "status": "passed"
});
formatter.match({
  "location": "NA45_Reseller_Login_stepDefs.iAccessCreateQuoteTabAndSelectTheAboveCompanyFromTheCompanyDropdown()"
});
formatter.result({
  "duration": 8204157532,
  "status": "passed"
});
formatter.match({
  "location": "NA45_Reseller_Login_stepDefs.theNewlyCreatedSiteShouldAppearUnderTheSiteDropDown()"
});
formatter.result({
  "duration": 1496211177,
  "status": "passed"
});
formatter.after({
  "duration": 499102226,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA47_WLR3_Permissions_For_Agent_Reseller.feature");
formatter.feature({
  "line": 1,
  "name": "Granting and Revoking WLR3 permissions for Agent and Reseller",
  "description": "",
  "id": "granting-and-revoking-wlr3-permissions-for-agent-and-reseller",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5358903086,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "CP revokes the WLR3 order permissions for an agent so that the WLR3 orders are hidden from the agent when he logs in and vice versa",
  "description": "",
  "id": "granting-and-revoking-wlr3-permissions-for-agent-and-reseller;cp-revokes-the-wlr3-order-permissions-for-an-agent-so-that-the-wlr3-orders-are-hidden-from-the-agent-when-he-logs-in-and-vice-versa",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "CP has revoked my WLR order permissions",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "I am logged in as agent",
  "keyword": "When "
});
formatter.step({
  "line": 7,
  "name": "I should not be able to see and edit them",
  "keyword": "Then "
});
formatter.step({
  "line": 8,
  "name": "the orders are visible and editable once the CP issues the orders back agent rights",
  "keyword": "And "
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.cpHasRevokedMyWLROrderPermissions()"
});
formatter.result({
  "duration": 18410121480,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 3518947135,
  "status": "passed"
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.iShouldNotBeAbleToSeeAndEditThem()"
});
formatter.result({
  "duration": 7895692194,
  "status": "passed"
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.theOrdersAreVisibleAndEditableOnceTheCPIssuesTheOrdersBack()"
});
formatter.result({
  "duration": 41291418199,
  "status": "passed"
});
formatter.after({
  "duration": 565103702,
  "status": "passed"
});
formatter.before({
  "duration": 4975664410,
  "status": "passed"
});
formatter.scenario({
  "line": 11,
  "name": "CP revokes the WLR3 order permissions for a reseller so that the WLR3 orders are hidden from the reseller when he logs in and vice versa",
  "description": "",
  "id": "granting-and-revoking-wlr3-permissions-for-agent-and-reseller;cp-revokes-the-wlr3-order-permissions-for-a-reseller-so-that-the-wlr3-orders-are-hidden-from-the-reseller-when-he-logs-in-and-vice-versa",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 10,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 12,
  "name": "CP has revoked my WLR order permissions",
  "keyword": "Given "
});
formatter.step({
  "line": 13,
  "name": "I am logged in as reseller",
  "keyword": "When "
});
formatter.step({
  "line": 14,
  "name": "I should not be able to see and edit them",
  "keyword": "Then "
});
formatter.step({
  "line": 15,
  "name": "the orders are visible and editable once the CP issues the orders back reseller rights",
  "keyword": "And "
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.cpHasRevokedMyWLROrderPermissions()"
});
formatter.result({
  "duration": 12952758194,
  "status": "passed"
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 3725462607,
  "status": "passed"
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.iShouldNotBeAbleToSeeAndEditThem()"
});
formatter.result({
  "duration": 7884187768,
  "status": "passed"
});
formatter.match({
  "location": "NA47_WLR3_Permissions_For_Agent_Reseller_stepDefs.theOrdersAreVisibleAndEditableOnceTheCPIssuesTheOrdersBackResellerRights()"
});
formatter.result({
  "duration": 39667839649,
  "status": "passed"
});
formatter.after({
  "duration": 427414345,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA49_OrderServiceAutomation.feature");
formatter.feature({
  "line": 1,
  "name": "order service automation for agent, reseller and CP. includes CP only and mandatory fields under select service.",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5259769102,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "Login as a CP, create a quote, add a service and verify the behaviour of CP only control",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-a-cp,-create-a-quote,-add-a-service-and-verify-the-behaviour-of-cp-only-control",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am logged in as CP",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 7,
  "name": "I add a service from the quote details page",
  "keyword": "When "
});
formatter.step({
  "line": 8,
  "name": "the control that is CP only should be visible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsCP()"
});
formatter.result({
  "duration": 3726612144,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 19377938670,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAddAServiceFromTheQuoteDetailsPage()"
});
formatter.result({
  "duration": 8589168814,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theControlThatIsCPOnlyShouldBeVisible()"
});
formatter.result({
  "duration": 2188886924,
  "status": "passed"
});
formatter.after({
  "duration": 430605457,
  "status": "passed"
});
formatter.before({
  "duration": 5268420818,
  "status": "passed"
});
formatter.scenario({
  "line": 11,
  "name": "Login as an agent, create a quote, add a service and verify the behaviour of CP only control",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-an-agent,-create-a-quote,-add-a-service-and-verify-the-behaviour-of-cp-only-control",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 10,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 12,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 13,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 14,
  "name": "I add a service from the quote details page",
  "keyword": "When "
});
formatter.step({
  "line": 15,
  "name": "the control that is CP only should be invisible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 3681938382,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 20186519626,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAddAServiceFromTheQuoteDetailsPage()"
});
formatter.result({
  "duration": 10082129711,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theControlThatIsCPOnlyShouldBeInvisible()"
});
formatter.result({
  "duration": 1713326962,
  "status": "passed"
});
formatter.after({
  "duration": 486147294,
  "status": "passed"
});
formatter.before({
  "duration": 5073694137,
  "status": "passed"
});
formatter.scenario({
  "line": 18,
  "name": "Login as a reseller, create a quote, add a service and verify the behaviour of CP only control",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-a-reseller,-create-a-quote,-add-a-service-and-verify-the-behaviour-of-cp-only-control",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 17,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 19,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 20,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 21,
  "name": "I add a service from the quote details page",
  "keyword": "When "
});
formatter.step({
  "line": 22,
  "name": "the control that is CP only should be invisible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 3982766303,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 20148023116,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAddAServiceFromTheQuoteDetailsPage()"
});
formatter.result({
  "duration": 9987595988,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theControlThatIsCPOnlyShouldBeInvisible()"
});
formatter.result({
  "duration": 1673066903,
  "status": "passed"
});
formatter.after({
  "duration": 510904602,
  "status": "passed"
});
formatter.before({
  "duration": 5341056886,
  "status": "passed"
});
formatter.scenario({
  "line": 25,
  "name": "Login as a CP, create a quote, add a service without selecting the mandatory control and check the behaviour",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-a-cp,-create-a-quote,-add-a-service-without-selecting-the-mandatory-control-and-check-the-behaviour",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 24,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 26,
  "name": "I am logged in as CP",
  "keyword": "Given "
});
formatter.step({
  "line": 27,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 28,
  "name": "I access quote details and add a service without selecting the mandatory control",
  "keyword": "When "
});
formatter.step({
  "line": 29,
  "name": "The quote should become invalid",
  "keyword": "Then "
});
formatter.step({
  "line": 30,
  "name": "When I add the omitted control, the quote should become valid",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsCP()"
});
formatter.result({
  "duration": 3932055359,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 19573429092,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAccessQuoteDetailsAndAddAServiceWithoutSelectingTheMandatoryControl()"
});
formatter.result({
  "duration": 11366951958,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theQuoteShouldBecomeInvalid()"
});
formatter.result({
  "duration": 927708974,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.whenIAddTheOmittedControlTheQuoteShouldBecomeValid()"
});
formatter.result({
  "duration": 4982499446,
  "status": "passed"
});
formatter.after({
  "duration": 466381476,
  "status": "passed"
});
formatter.before({
  "duration": 5262681585,
  "status": "passed"
});
formatter.scenario({
  "line": 33,
  "name": "Login as an agent, create a quote, add a service without selecting the mandatory control and check the behaviour",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-an-agent,-create-a-quote,-add-a-service-without-selecting-the-mandatory-control-and-check-the-behaviour",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 32,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 34,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 35,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 36,
  "name": "I access quote details and add a service without selecting the mandatory control",
  "keyword": "When "
});
formatter.step({
  "line": 37,
  "name": "The quote should become invalid",
  "keyword": "Then "
});
formatter.step({
  "line": 38,
  "name": "When I add the omitted control, the quote should become valid",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 3891290869,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 20236803720,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAccessQuoteDetailsAndAddAServiceWithoutSelectingTheMandatoryControl()"
});
formatter.result({
  "duration": 12092280533,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theQuoteShouldBecomeInvalid()"
});
formatter.result({
  "duration": 1039995013,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.whenIAddTheOmittedControlTheQuoteShouldBecomeValid()"
});
formatter.result({
  "duration": 4995758366,
  "status": "passed"
});
formatter.after({
  "duration": 599646264,
  "status": "passed"
});
formatter.before({
  "duration": 5271877881,
  "status": "passed"
});
formatter.scenario({
  "line": 41,
  "name": "Login as a reseller, create a quote, add a service without selecting the mandatory control and check the behaviour",
  "description": "",
  "id": "order-service-automation-for-agent,-reseller-and-cp.-includes-cp-only-and-mandatory-fields-under-select-service.;login-as-a-reseller,-create-a-quote,-add-a-service-without-selecting-the-mandatory-control-and-check-the-behaviour",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 40,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 42,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 43,
  "name": "Have created a quote",
  "keyword": "And "
});
formatter.step({
  "line": 44,
  "name": "I access quote details and add a service without selecting the mandatory control",
  "keyword": "When "
});
formatter.step({
  "line": 45,
  "name": "The quote should become invalid",
  "keyword": "Then "
});
formatter.step({
  "line": 46,
  "name": "When I add the omitted control, the quote should become valid",
  "keyword": "And "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 4834082384,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.haveCreatedAQuote()"
});
formatter.result({
  "duration": 21113337769,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.iAccessQuoteDetailsAndAddAServiceWithoutSelectingTheMandatoryControl()"
});
formatter.result({
  "duration": 10208870091,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.theQuoteShouldBecomeInvalid()"
});
formatter.result({
  "duration": 1100823573,
  "status": "passed"
});
formatter.match({
  "location": "NA49_OrderServiceAutomation_stepDefs.whenIAddTheOmittedControlTheQuoteShouldBecomeValid()"
});
formatter.result({
  "duration": 4781244434,
  "status": "passed"
});
formatter.after({
  "duration": 446202694,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA51_Service Designer application_Familiarise.feature");
formatter.feature({
  "line": 1,
  "name": "verify customer and internal fields by logging as CP, agent and reseller",
  "description": "",
  "id": "verify-customer-and-internal-fields-by-logging-as-cp,-agent-and-reseller",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5469801409,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "verify that the customer and internal tabs are present and accessible under add/view notes under edit order page",
  "description": "",
  "id": "verify-customer-and-internal-fields-by-logging-as-cp,-agent-and-reseller;verify-that-the-customer-and-internal-tabs-are-present-and-accessible-under-add/view-notes-under-edit-order-page",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am logged in as CP",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "I access add_view notes on edit order page",
  "keyword": "When "
});
formatter.step({
  "line": 7,
  "name": "customer and internal tabs should be present and accessible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsCP()"
});
formatter.result({
  "duration": 3917562378,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iAccessAdd_viewNotesOnEditOrderPage()"
});
formatter.result({
  "duration": 29372394750,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.customerAndInternalTabsShouldBePresentAndAccessible()"
});
formatter.result({
  "duration": 3054516370,
  "status": "passed"
});
formatter.after({
  "duration": 745415285,
  "status": "passed"
});
formatter.before({
  "duration": 5301964477,
  "status": "passed"
});
formatter.scenario({
  "line": 10,
  "name": "verify the customer and internal tabs are absent under add/view notes under edit order page",
  "description": "",
  "id": "verify-customer-and-internal-fields-by-logging-as-cp,-agent-and-reseller;verify-the-customer-and-internal-tabs-are-absent-under-add/view-notes-under-edit-order-page",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 9,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 11,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 12,
  "name": "I access add_view notes on edit order page",
  "keyword": "When "
});
formatter.step({
  "line": 13,
  "name": "customer and internal tabs should be absent",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 4598547140,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iAccessAdd_viewNotesOnEditOrderPage()"
});
formatter.result({
  "duration": 31428243895,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.customerAndInternalTabsShouldBeAbsent()"
});
formatter.result({
  "duration": 2770005657,
  "status": "passed"
});
formatter.after({
  "duration": 493131757,
  "status": "passed"
});
formatter.before({
  "duration": 5024125485,
  "status": "passed"
});
formatter.scenario({
  "line": 16,
  "name": "verify the customer and internal tabs are absent under add/view notes under edit order page",
  "description": "",
  "id": "verify-customer-and-internal-fields-by-logging-as-cp,-agent-and-reseller;verify-the-customer-and-internal-tabs-are-absent-under-add/view-notes-under-edit-order-page",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 15,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 17,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 18,
  "name": "I access add_view notes on edit order page",
  "keyword": "When "
});
formatter.step({
  "line": 19,
  "name": "customer and internal tabs should be absent",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 3832851039,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iAccessAdd_viewNotesOnEditOrderPage()"
});
formatter.result({
  "duration": 32181162566,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.customerAndInternalTabsShouldBeAbsent()"
});
formatter.result({
  "duration": 2719796730,
  "status": "passed"
});
formatter.after({
  "duration": 489403008,
  "status": "passed"
});
formatter.before({
  "duration": 5716373778,
  "status": "passed"
});
formatter.scenario({
  "line": 22,
  "name": "assert that an assigned service is unavailable to agent and vice versa",
  "description": "",
  "id": "verify-customer-and-internal-fields-by-logging-as-cp,-agent-and-reseller;assert-that-an-assigned-service-is-unavailable-to-agent-and-vice-versa",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 21,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 23,
  "name": "I am logged in as agent and there is a service unassigned to me",
  "keyword": "Given "
});
formatter.step({
  "line": 24,
  "name": "I access the services page",
  "keyword": "When "
});
formatter.step({
  "line": 25,
  "name": "I should not be able to see that particular service",
  "keyword": "Then "
});
formatter.step({
  "line": 26,
  "name": "I should be able to see the service once CP assigns it to me",
  "keyword": "And "
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iAmLoggedInAsAgentAndThereIsAServiceUnassignedToMe()"
});
formatter.result({
  "duration": 23291426350,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iAccessTheServicesPage()"
});
formatter.result({
  "duration": 9053416039,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iShouldNotBeAbleToSeeThatParticularService()"
});
formatter.result({
  "duration": 128476459,
  "status": "passed"
});
formatter.match({
  "location": "NA51_ServiceDesignerApplication_Familiarise_stepDefs.iShouldBeAbleToSeeTheServiceOnceCPAssignsItToMe()"
});
formatter.result({
  "duration": 30247887937,
  "status": "passed"
});
formatter.after({
  "duration": 611196876,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.WLR3.feature/NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller.feature");
formatter.feature({
  "line": 1,
  "name": "Accessing controls in services with different permissions (CP, Agent and reseller)",
  "description": "",
  "id": "accessing-controls-in-services-with-different-permissions-(cp,-agent-and-reseller)",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5336231427,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "verify that a charge designated CP only is accessible by CP",
  "description": "",
  "id": "accessing-controls-in-services-with-different-permissions-(cp,-agent-and-reseller);verify-that-a-charge-designated-cp-only-is-accessible-by-cp",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am logged in as CP",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "create a new quote and access a service",
  "keyword": "And "
});
formatter.step({
  "line": 7,
  "name": "the CP only control should be visible and accessible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsCP()"
});
formatter.result({
  "duration": 3840233976,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.createANewQuoteAndAccessAService()"
});
formatter.result({
  "duration": 33242272775,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.theCPOnlyControlShouldBeVisibleAndAccessible()"
});
formatter.result({
  "duration": 718845525,
  "status": "passed"
});
formatter.after({
  "duration": 629766066,
  "status": "passed"
});
formatter.before({
  "duration": 5174586236,
  "status": "passed"
});
formatter.scenario({
  "line": 10,
  "name": "verify that a charge designated CP only is inaccessible by agent",
  "description": "",
  "id": "accessing-controls-in-services-with-different-permissions-(cp,-agent-and-reseller);verify-that-a-charge-designated-cp-only-is-inaccessible-by-agent",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 9,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 11,
  "name": "I am logged in as agent",
  "keyword": "Given "
});
formatter.step({
  "line": 12,
  "name": "create a new quote and access a service",
  "keyword": "And "
});
formatter.step({
  "line": 13,
  "name": "the CP only control should be invisible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsAgent()"
});
formatter.result({
  "duration": 3636975426,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.createANewQuoteAndAccessAService()"
});
formatter.result({
  "duration": 25523909343,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.theCPOnlyControlShouldBeInvisible()"
});
formatter.result({
  "duration": 7311561964,
  "status": "passed"
});
formatter.after({
  "duration": 556244600,
  "status": "passed"
});
formatter.before({
  "duration": 5253520004,
  "status": "passed"
});
formatter.scenario({
  "line": 16,
  "name": "verify that a charge designated CP only is inaccessible by reseller",
  "description": "",
  "id": "accessing-controls-in-services-with-different-permissions-(cp,-agent-and-reseller);verify-that-a-charge-designated-cp-only-is-inaccessible-by-reseller",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 15,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 17,
  "name": "I am logged in as reseller",
  "keyword": "Given "
});
formatter.step({
  "line": 18,
  "name": "create a new quote and access a service",
  "keyword": "And "
});
formatter.step({
  "line": 19,
  "name": "the CP only control should be invisible",
  "keyword": "Then "
});
formatter.match({
  "location": "NA39_BDD_Scenarios_Creation_Of_User_Agent_Reseller_permissions_stepdefs.iAmLoggedInAsReseller()"
});
formatter.result({
  "duration": 3734887120,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.createANewQuoteAndAccessAService()"
});
formatter.result({
  "duration": 32382962461,
  "status": "passed"
});
formatter.match({
  "location": "NA52_Accessing_controls_In_Services_with_different_permissions_CP_Agent_Reseller_stepDefs.theCPOnlyControlShouldBeInvisible()"
});
formatter.result({
  "duration": 1547740200,
  "status": "passed"
});
formatter.after({
  "duration": 577498055,
  "status": "passed"
});
formatter.uri("com/unionstreet/NxTier/com.unionstreet.NxTier.smokePack.feature/NxTier_smokePack.feature");
formatter.feature({
  "line": 1,
  "name": "Creating new business customer and assigning all the required inputs to generate an invoice bill",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill",
  "keyword": "Feature"
});
formatter.before({
  "duration": 5470814499,
  "status": "passed"
});
formatter.scenario({
  "line": 4,
  "name": "Verify that login is successful with valid username and password",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;verify-that-login-is-successful-with-valid-username-and-password",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 3,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 5,
  "name": "I am on login page",
  "keyword": "Given "
});
formatter.step({
  "line": 6,
  "name": "I enter valid username and password",
  "keyword": "When "
});
formatter.step({
  "line": 7,
  "name": "Login is successful",
  "keyword": "Then "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmOnLoginPage()"
});
formatter.result({
  "duration": 19949056,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iEnterValidUsernameAndPassword()"
});
formatter.result({
  "duration": 3863820919,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.LoginIsSuccessful()"
});
formatter.result({
  "duration": 2619580528,
  "status": "passed"
});
formatter.after({
  "duration": 596341345,
  "status": "passed"
});
formatter.scenarioOutline({
  "line": 10,
  "name": ": Login with invalid credentials",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;:-login-with-invalid-credentials",
  "type": "scenario_outline",
  "keyword": "Scenario Outline",
  "tags": [
    {
      "line": 9,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 11,
  "name": "I am on login page",
  "keyword": "Given "
});
formatter.step({
  "line": 12,
  "name": "I enter invalid \"\u003cuserName\u003e\" and \"\u003cpassWord\u003e\"",
  "keyword": "When "
});
formatter.step({
  "line": 13,
  "name": "The login should fail",
  "keyword": "Then "
});
formatter.examples({
  "line": 15,
  "name": "",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;:-login-with-invalid-credentials;",
  "rows": [
    {
      "cells": [
        "userName",
        "passWord"
      ],
      "line": 16,
      "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;:-login-with-invalid-credentials;;1"
    },
    {
      "cells": [
        "john",
        "abcdef"
      ],
      "line": 17,
      "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;:-login-with-invalid-credentials;;2"
    }
  ],
  "keyword": "Examples"
});
formatter.before({
  "duration": 5597930109,
  "status": "passed"
});
formatter.scenario({
  "line": 17,
  "name": ": Login with invalid credentials",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;:-login-with-invalid-credentials;;2",
  "type": "scenario",
  "keyword": "Scenario Outline",
  "tags": [
    {
      "line": 9,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 11,
  "name": "I am on login page",
  "keyword": "Given "
});
formatter.step({
  "line": 12,
  "name": "I enter invalid \"john\" and \"abcdef\"",
  "matchedColumns": [
    0,
    1
  ],
  "keyword": "When "
});
formatter.step({
  "line": 13,
  "name": "The login should fail",
  "keyword": "Then "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmOnLoginPage()"
});
formatter.result({
  "duration": 17364711,
  "status": "passed"
});
formatter.match({
  "arguments": [
    {
      "val": "john",
      "offset": 17
    },
    {
      "val": "abcdef",
      "offset": 28
    }
  ],
  "location": "NxTier_smokePack_stepDefs.iEnterInvalidAnd(String,String)"
});
formatter.result({
  "duration": 1112909410,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.TheLoginShouldFail()"
});
formatter.result({
  "duration": 20423300,
  "status": "passed"
});
formatter.after({
  "duration": 444257416,
  "status": "passed"
});
formatter.before({
  "duration": 5499122754,
  "status": "passed"
});
formatter.scenario({
  "line": 20,
  "name": "Creating a new Business Customer",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;creating-a-new-business-customer",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 19,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 21,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 22,
  "name": "I access add new business customer page",
  "keyword": "When "
});
formatter.step({
  "line": 23,
  "name": "Provide all the required info",
  "keyword": "And "
});
formatter.step({
  "line": 24,
  "name": "A new customer should be successfully created",
  "keyword": "Then "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 4073846282,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAccessAddNewBusinessCustomerPage()"
});
formatter.result({
  "duration": 3725665164,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.provideAllTheRequiredInfo()"
});
formatter.result({
  "duration": 4239143244,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.aNewCustomerShouldBeSuccessfullyCreated()"
});
formatter.result({
  "duration": 5370239971,
  "status": "passed"
});
formatter.after({
  "duration": 445192925,
  "status": "passed"
});
formatter.before({
  "duration": 4863761751,
  "status": "passed"
});
formatter.scenario({
  "comments": [
    {
      "line": 26,
      "value": "#  @smokePack"
    },
    {
      "line": 27,
      "value": "#  Scenario: Creating a sub-site for a newly created customer"
    },
    {
      "line": 28,
      "value": "#    Given I am logged in as an admin"
    },
    {
      "line": 29,
      "value": "#    And I have added a new business customer"
    },
    {
      "line": 30,
      "value": "#    When I access Company Sites under the company menu and provide all the required info"
    },
    {
      "line": 31,
      "value": "#    Then A new site should appear under the respective company\u0027s menu"
    },
    {
      "line": 33,
      "value": "#  @smokePack"
    },
    {
      "line": 34,
      "value": "#  Scenario: Adding invoice details and billing information to a newly created customer"
    },
    {
      "line": 35,
      "value": "#    Given I am logged in as an admin"
    },
    {
      "line": 36,
      "value": "#    And   I have added a new business customer"
    },
    {
      "line": 37,
      "value": "#    When  I access invoicing and billing details section under company menu and provide all required info"
    },
    {
      "line": 38,
      "value": "#    Then  Invoice details and billing details of that particular customer should be updated, respectively"
    }
  ],
  "line": 41,
  "name": "Adding service charges (one off and recurring) and CLI to the newly created customer",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;adding-service-charges-(one-off-and-recurring)-and-cli-to-the-newly-created-customer",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 40,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 42,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 43,
  "name": "I have added a new business customer",
  "keyword": "And "
});
formatter.step({
  "line": 44,
  "name": "I access CLIs and service charges section under company menu and provide all required info",
  "keyword": "When "
});
formatter.step({
  "line": 45,
  "name": "CLIs and service charges of that particular customer should be updated, respectively",
  "keyword": "Then "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 4651903709,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iHaveAddedANewBusinessCustomer()"
});
formatter.result({
  "duration": 7609199074,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAccessCLIsAndServiceChargesSectionUnderCompanyMenuAndProvideAllRequiredInfo()"
});
formatter.result({
  "duration": 28365093922,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.clisAndServiceChargesOfThatParticularCustomerShouldBeUpdatedRespectively()"
});
formatter.result({
  "duration": 8126487711,
  "status": "passed"
});
formatter.after({
  "duration": 422670995,
  "status": "passed"
});
formatter.before({
  "duration": 4699401504,
  "status": "passed"
});
formatter.scenario({
  "line": 48,
  "name": "Adding pricing details to a newly created customer",
  "description": "",
  "id": "creating-new-business-customer-and-assigning-all-the-required-inputs-to-generate-an-invoice-bill;adding-pricing-details-to-a-newly-created-customer",
  "type": "scenario",
  "keyword": "Scenario",
  "tags": [
    {
      "line": 47,
      "name": "@smokePack"
    }
  ]
});
formatter.step({
  "line": 49,
  "name": "I am logged in as an admin",
  "keyword": "Given "
});
formatter.step({
  "line": 50,
  "name": "I have added a new business customer",
  "keyword": "And "
});
formatter.step({
  "line": 51,
  "name": "I access pricing details under company menu and provide all required info",
  "keyword": "When "
});
formatter.step({
  "line": 52,
  "name": "Pricing details of that particular customer  should be updated",
  "keyword": "Then "
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAmLoggedInAsAnAdmin()"
});
formatter.result({
  "duration": 5636620723,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iHaveAddedANewBusinessCustomer()"
});
formatter.result({
  "duration": 7192478321,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.iAccessPricingDetailsUnderCompanyMenuAndProvideAllRequiredInfo()"
});
formatter.result({
  "duration": 11179764586,
  "status": "passed"
});
formatter.match({
  "location": "NxTier_smokePack_stepDefs.pricingDetailsOfThatParticularCustomerShouldBeUpdated()"
});
formatter.result({
  "duration": 2610202201,
  "status": "passed"
});
formatter.after({
  "duration": 535517313,
  "status": "passed"
});
});